...
If the host and target do not match, the loader script aborts with an explicit error
before downloading anything. Building arm64 Lambdas from a non-arm64 host requires
running the module from inside a matching container (e.g. linux/arm64).
